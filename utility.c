...
// resets the text color to the default color for the terminal.

int back_bat = 0;		  // indicate whether a process is both a background process and a batch process. 1=back , 0=not, it means it is both background process and batch, effective value of 1
int output_num = 0;		  // used to store the number of output batch numbers in redirection., the number of output batch number in redirection
char batchfile[MAX_PATH]; // used to store the name of the current batch file.,
int bat_num = 0;		  // used to store the number of batch command-lines,
int isBatch = 0;		  // used to indicate whether the current process is a batch process 1 = yes , 0 = not
int letter;				  // store a single letter.
char *open;				  // store the name of a file that is to be opened
...
int Execute(char *buffer) // buffer contain a command to be executed
{
	pid_t pid;
	...
	Redirect Inputs[MAX_OPEN];	//   input    redirection (10) max files can be oppened at once
	Redirect Outputs[MAX_OPEN]; //   output   redirection   (10)

	error = Command_strtok(buffer, args, states, Inputs, Outputs);
	/*
	
	tokenizing the command in buffer into individual arguments,
	and storing the resulting array of strings in args.
	It also populates the states array and the Inputs and Outputs arrays
	with information about the command.
	 :tokenize  input last  entry  will  be  NULL
	 */
	if (error || args[0] == NULL)
		return -1; // If the input format error	or  if there's anything there

	if (!strcmp(args[0], "quit") || !strcmp(args[0], "exit")) // if entered "quit" command
	{
		//It checks if there is an argument after the quit command.
	...
	else if (states[0]) // states[0]==1  running in background "flag indicate whether executed in background or not"
	{
		switch (pid = fork())
		{
		case -1:
			Error(-9, NULL, NULL, states, "fork");//print error message and exit prog
...
int Command_exec(char **args, const Redirect *Inputs, const Redirect *Outputs, int *states)
{
	...
	if (!strcmp(args[0], "myshell") || !strcmp(args[0], "shell")) // strcmp() cmp function "myshell" command, checks if the first argument s either "myshell" or "shell"
	{
		flag = 0;
		if (isBatch) //     e.g.   execute myshell b.txt in test.bat
		{
			switch (newpid = fork())
			{
			case -1:
			...
			case 0:
			// is both a background process and a batch process.
	...
	else if (strcmp(args[0], "clr") == 0 || strcmp(args[0], "clear") == 0) // "clear"  command
	{																	   // system("clear");
		if (output_num == 0)
...
{
	int i;
	if (isBatch) // if executes from batchfile
	...
	case 0:		 
		fprintf(stderr, " %s\n", msg);
		break;
	//Prints a message indicating an invalid argument was encountered while trying to open a file.
	case 1:		
		...
		break;
	//Prints a message indicating an invalid argument was encountered after a redirection operator.
	case 2: 	
		fprintf(stderr, "Format Error(Letter %d): '%s' followed by invlid argument '%c'\n", letter, open, *msg); // <  >> >
		fprintf(stderr, "\nType 'help redirection'  to get help information abbout '<'  '>>' and '>'\n");
		break;
	//Prints a message indicating that no help information was found for the command passed in msg.
	case 3:  	
		...
		break;
	//Prints a message indicating that no arguments are needed after the command passed in msg.	
	case 4:		
		...
		break;
	//Prints a message indicating that the maximum number of files that can be opened has been reached.
	case 5: 	
		fprintf(stderr, "\nSystem Note: can not open more than %d files as %s\n", MAX_OPEN, msg);
		break;
	//Prints a message indicating that the command passed in args is not an internal command or an executable file.
	case -1: 	
		fprintf(stderr, "\nSystem Warning: \"");
		while (*args)
			fprintf(stderr, "%s ", *args++);
		fprintf(stderr, "\b\" is not internal command or executive file\n");
		if (isBatch == 0 && output_num == 0) //
			fprintf(stderr, "\nType \"help command\" to see supported internal commands.\n");
		break; // abort( );
	//	Prints a message indicating that the arguments passed in args are invalid after the command passed in msg.
	case -2: 	
		...
		break;
	//Invalid input redirection detected .
	case -3: 	
		...
		break;
	//Prints a message indicating that invalid output redirection has occurred.
	case -4: 	
	...
	case -5: 	
		...
		break;
	// there is an error opening a file.
	case -6:	 
		fprintf(stderr, "File Error: can not open file \"%s\"\n", msg);
		break;
	//overflow error due to a directory path being longer than the maximum permitted length.
	case -7: 	
...
/*   Function "Command_strtok" :  analyse the command and parse it into program and commands
...
int Command_strtok(char *buf, char **args, int *states, Redirect *Inputs, Redirect *Outputs) // redirection of InPut & OutPuts
{
	...
	while (buf[++i] && buf[i] != '#') // scan the command, "#" for remark
	{
		...
		{
		//it indicates the start of an input redirection, and the function processes the input redirection accordingly.
		case '<': 
			letter++;
			if (flag == 0)
			{
			...
			open = "<"; 
			while (buf[++i] == ' ' || buf[i] == '\t')
				; // Skip a number of consecutive spaces
			...
			i--;
			break;
		// "r" opentype, it indicates the start of an output redirection, and the function processes the output redirection accordingly
		case '>':
			letter++;
			if (flag == 0)
			{
...
int Command_shell(FILE *inputfile, const Redirect *Outputs, const int *states) //
{
	...
	{  /* get command line from input */
		...
		{
			printf(ANSI_COLOR_BIRGHT_BLUE "%s@%s" ANSI_COLOR_RESET, getenv("USER"), "hurih"); // Prints the name of the shell
			printf(":");
			printf(ANSI_COLOR_YELLOW_BOLD "%s" ANSI_COLOR_RESET, getenv("PWD")); // Prints relative directory
			printf("$ ");
		} // write prompt
...
{																						   //   fprintf(stderr,"mybat");
	...
	if (args[1]) // e.g.  myshell test.bat    myshell <a.bxt
	{
		...
		if (isBatch && !strcmp(fullpath_batchfile, filepath)) // e.g.  myshell a.txt, a.txt has the command line of myshell a.txt
		{
